...
var (
	...
	mdWorkspaceTokensEstimatedDesc = prometheus.NewDesc(
		prometheus.BuildFQName(namespace, "md", "workspace_tokens_estimated"),
		"Total estimated tokens in workspace.",
		[]string{"workspace"}, nil,
	)
)
...
func NewWorkspaceCollector(home string) prometheus.Collector {
	descs := []*prometheus.Desc{
		...
		mdWorkspaceTokensEstimatedDesc,
	}
...
func collectWorkspaceMetrics(home string) []prometheus.Metric {
	...
	for wsDir, agent := range WorkspaceMap {
		...
		metrics = append(metrics,
			...
			prometheus.MustNewConstMetric(mdWorkspaceTokensEstimatedDesc, prometheus.GaugeValue, totalTokens, agent),
		)
	}
