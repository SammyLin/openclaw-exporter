"""Metric definitions and collection functions."""
...
def get_cron_jobs(openclaw_home):
...
def collect_cron_jobs(openclaw_home, prev_job_keys):
    """Collect cron job metrics. Returns set of current (job_name, job_id) tuples."""
    jobs = get_cron_jobs(openclaw_home)
    current_keys = set()
...
def _load_sessions(openclaw_home, agent_name):
    """Load sessions.json for an agent, return list of session dicts."""
    path = os.path.join(openclaw_home, "agents", agent_name, "sessions", "sessions.json")
...
def _read_session_usage(openclaw_home, agent_name, session_id):
    """Read usage records from a session JSONL. Returns aggregated totals."""
    path = os.path.join(
        openclaw_home, "agents", agent_name, "sessions", f"{session_id}.jsonl"
    )
    if not os.path.isfile(path):
        return None
...
def collect_cron_session_tokens(openclaw_home):
    ...
    for agent in AGENT_NAMES:
        ...
        for cron_name, s in cron_sessions.items():
            usage = _read_session_usage(openclaw_home, agent, s["sessionId"])
            if not usage:
                continue
...
def collect_agent_session_tokens(openclaw_home):
